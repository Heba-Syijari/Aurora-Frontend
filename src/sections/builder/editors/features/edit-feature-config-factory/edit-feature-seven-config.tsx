...
import { FeatureSevenConfig } from 'src/sections/builder/components/featuers/types';
...
export interface EditFeatureSevenConfigProps {
  defaultValue: FeatureSevenConfig;
  onSave: (value: FeatureSevenConfig) => void;
  theme: Theme;
  onCloseClicked: () => void;
}
...
export default function FeatureEditConfig({
  ...
  onCloseClicked,
  theme,
}: EditFeatureSevenConfigProps) {
  const paletteColorsMap = useMemo(() => getPaletteColorsMap(theme.palette), [theme.palette]);

  const schema = Yup.object().shape({
    lightTextColor: Yup.string().required('This field is required'),
    darkTextColor: Yup.string().required('This field is required'),
    lightBackgroundColor: Yup.string().required('This field is required'),
    darkbackgroundColor: Yup.string().required('This field is required'),
    features: Yup.array()
      .of(
        Yup.object().shape({
  ...
  const methods = useForm({ resolver: yupResolver<FeatureSevenConfig>(schema), defaultValues });
  ...
  return (
    <FormProvider id="edit-feature-config-form" methods={methods} onSubmit={onSubmit}>
      <Stack spacing={2} sx={{ py: 2 }}>
        ...
        <FormGroup label="Dark Background color">
          <RHFColorsGroup
        ...
        <FormGroup label="Light Background color">
          <RHFColorsGroup
