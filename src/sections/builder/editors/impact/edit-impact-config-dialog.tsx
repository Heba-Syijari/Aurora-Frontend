...
interface Props {
  ...
  open: boolean;
  onCloseClicked: () => void;
  onSave: (value: ImpactConfig) => void;
}

export function EditImpactConfigDialog({ defaultValue, open, onCloseClicked, onSave }: Props) {
      ...
      <DialogContent>
        <FormProvider id="edit-faq-config-form" methods={methods} onSubmit={onSubmit}>
          <Stack spacing={2} sx={{ py: 2 }}>
            <FormGroup label="Impact title color">
              <RHFColorsGroup
                name="titleTextColor"
                row
            ...
            <FormGroup label="Impact description color">
              <RHFColorsGroup
            ...
            <Box sx={{ pt: 2 }}>
              <RHFDynamicMultipleTextFields
                name="impacts"
                defaultValuesForNewField={{ title: '', description: '' }}
