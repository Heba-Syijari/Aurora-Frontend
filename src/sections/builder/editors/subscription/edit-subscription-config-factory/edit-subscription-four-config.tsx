...
import { SubscriptionFourConfig } from 'src/sections/builder/components/subscription/types';
...
export interface EditSubscriptionFourConfigProps {
  defaultValue: SubscriptionFourConfig;
  onSave: (value: SubscriptionFourConfig) => void;
  theme: Theme;
  onCloseClicked: () => void;
}

export default function EditSubscriptionFourConfig({
  defaultValue,
  onSave,
  onCloseClicked,
  theme,
}: EditSubscriptionFourConfigProps) {
    ...
    subscriptions: Yup.array()
      .of(
        Yup.object().shape({
          ...
          icon: Yup.string().nullable().required('This field is required'),
  ...
  const methods = useForm({ resolver: yupResolver<SubscriptionFourConfig>(schema), defaultValues });
